...
def append_to_json(filename, item):
    ...
    h_urls = [_.get('H_url') for _ in json_data]
    
    if (item['H_chargers_fee'] == '' and item['feedback_data'] != [] and item['feedback_data'] != [[]]) and (item['H_url'] not in h_urls):
...
def read_file(filename):
    with open(filename, 'r', encoding='utf-8-sig') as file:
        data = json.load(file)
    item_urls = [d_item['H_url'] for d_item in data]
...
def get_one_data(url, filter_index, page):
    print(f'正在爬取第{filter_index}类（16）的第{page}页。{url}')
    headers = {
...
def first_get(start_index=5):
    ...
    for filter_index, c in enumerate(c_list[start_index:], start=start_index+1):
        for page in range(1, 501):
            ...
            urls = ['https://www.workaway.info' + _ for _ in
                    select.xpath('//a[@class="listentry-title"]/@href').getall()]
            for url in urls:
                item = get_one_data(url, filter_index, page)
                append_to_json(filename, item)

# 保存文件名
filename = '6th_Filtered_workaway.json'
first_get()
